...
public class EntityDaoImpl<T extends BaseEntity> implements EntityDao<T> {
    ...
    @Override
    public List<T> listEntitiesById(Long id, Page page, Class<T> entityClass) {
    ...
    public List<T> listEntities(Page page, Class<T> entityClass) {
        ...
        String realSql = "select name, age, gender from person order by age desc";

        String realSql2 = "select name, Sno from student order by age Sno";
    ...
    public T getEntity(T entity) {
        List<T> list = listEntitiesByEntity(entity);
        if (list.size() >= 1) {
            return list.get(0);
        }
    ...
    @Override
    public List<T> listEntitiesByEntity(T entity) {
        List<T> result = null;
        String sql;
    ...
    @Override
    public Long getTotalNumber(Class<T> entityClass) {
        try {
            ...
            if (resultSet.next() ) {
                ...
                return (Long) resultSet.getObject("total");
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
